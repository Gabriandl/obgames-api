...
public class CategoriaController {
    ...
    public ResponseEntity<Optional<Categoria>> updateCategoria(@PathVariable String id, @RequestBody Categoria categoria) {
        categoria.setId(id);
        return ResponseEntity.ok().body(this.categoriaService.updateCategoria(categoria, id));
    }

    @DeleteMapping("/categorias/{id}")
    public HttpStatus deleteCategoria(@PathVariable String id) {
